...
4. **Permission gate:** check `author_association` — only `OWNER`,
   `MEMBER`, or `COLLABORATOR` may use any `/skill-litmus` command.
   Reject others with ❌ and reply: "Only repository collaborators
   can use /skill-litmus commands."
5. Route to the appropriate handler.
6. React on the comment: 👀 when processing starts, 👍 when done,
   👎 on error.

**Rationale for gating all commands.** The `feedback` command commits
to the PR branch, so untrusted commenters could inject adversarial
feedback text that later flows into the `iterate` prompt. The `rerun`
command consumes Anthropic API credits. Gating all commands to
collaborators provides a uniform trust boundary rather than
per-command rules that are harder to reason about.

### 3.2 Slash Commands

#### `/skill-litmus feedback [skill-name]`

Captures per-eval human feedback from a PR comment.

**Skill targeting.** In multi-skill repos, the skill name is required.
In single-skill repos, it is auto-detected. If multiple skills exist
and no name is provided, the command replies with an error listing
available skills. If a skill name is provided but not found in the
repo, the command replies with an error listing available skills.
The skill name is validated against `evals/<skill-name>/evals.json`.

**Input format.** The eval results PR review includes a bot-generated
feedback template listing all eval IDs and the skill name. The user
copies the template, fills in notes, and posts it as a comment:
...
#### `/skill-litmus iterate [skill-name]`

Runs develop-eval in CI to propose SKILL.md improvements based on
captured feedback. Skill targeting follows the same rules as
...
- **Permission gating.** All `/skill-litmus` commands are gated to
  repository collaborators (`OWNER`, `MEMBER`, `COLLABORATOR`) to
  prevent untrusted commenters from injecting adversarial feedback,
  consuming API credits, or triggering LLM-driven iteration.
- **Prompt injection via eval outputs.** The `iterate` command feeds
  ...
  Mitigations: collaborator-only permission gate, changes are never
  auto-applied (posted as review comment with diff), human reviews
  proposed changes before applying. `Bash` is excluded from allowed
  tools — only `Read` and `Glob` are permitted.
- **Comment body isolation.** The comment body is written to a temp
  file via an environment variable in a separate workflow step, never
  interpolated into shell script text. This prevents the classic
  GitHub Actions `${{ }}` script injection vulnerability.
...
- **Skill name validation.** Skill names from both user commands and
  `evals.json` files are validated against `^[a-zA-Z0-9_-]+$`. In
  auto-detect mode, skills are also rejected if the `skill_name`
  field inside `evals.json` does not match its parent directory name.
  This prevents an attacker from adding a crafted `evals.json` with
  a path-traversal `skill_name` that differs from its directory.
